...
printf "${BLUE} 本脚本支持：macOS 10.13+ / Debian9+ / Ubuntu16.04+ / Centos7+${NC}\n"
printf "${BLUE} 原创：www.v2rayssr.com （已开启禁止国内访问）${NC}\n"
printf "${BLUE} YouTube频道：波仔分享${NC}\n"
printf "${BLUE} 本脚本禁止在国内任何网站转载${NC}\n"
printf "${GREEN}==========================================================${NC}\n"

# 默认密钥文件路径（调整为 macOS 用户目录）
DEFAULT_KEY_FILE="$HOME/1.key"
DEFAULT_CERT_FILE="$HOME/1.crt"

check_acme_installation() {
    if [ -d "$HOME/.acme.sh" ]; then
        if [ -f "$HOME/.acme.sh/account.conf" ]; then
            CF_KEY=$(grep 'SAVED_CF_Key' "$HOME/.acme.sh/account.conf" | cut -d'=' -f2)
            CF_EMAIL=$(grep 'SAVED_CF_Email' "$HOME/.acme.sh/account.conf" | cut -d'=' -f2)
...
install_socat() {
    if [[ "$(uname)" == "Darwin" ]]; then
        if ! command -v brew >/dev/null 2>&1; then
            ...
            printf "${BLUE}安装 Homebrew 命令：/bin/bash -c \"\$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\"${NC}\n"
            exit 1
        fi
        brew install socat
        ...
        printf "${RED}未知的操作系统，无法安装 socat。${NC}\n"
        exit 1
    fi
}
...
install_acme() {
    ...
    if [ -f "$HOME/.acme.sh/acme.sh" ]; then
        printf "${GREEN}acme.sh 安装成功。${NC}\n"
    else
        printf "${RED}acme.sh 安装失败，请检查网络或手动安装。${NC}\n"
        exit 1
    fi
}
...
write_cloudflare_config() {
    # macOS 的 sed 需要 -i '' 来避免备份文件
    sed -i '' "\$a\\
SAVED_CF_Key=$API_KEY" "$HOME/.acme.sh/account.conf"
    sed -i '' "\$a\\
SAVED_CF_Email=$EMAIL" "$HOME/.acme.sh/account.conf"
}
...
install_certificate() {
    "$HOME/.acme.sh/acme.sh" --installcert -d "*.$DOMAIN" --key-file "$KEY_FILE" --fullchain-file "$CERT_FILE"
}
...
main() {
    ...
    write_cloudflare_config
    request_certificate
    if [ -f "$HOME/.acme.sh/*.$DOMAIN_ecc/ca.cer" ]; then
        install_certificate
        printf "${GREEN}证书申请成功。${NC}\n"
        printf "${GREEN}您的证书文件: ${CERT_FILE}${NC}\n"
        printf "${GREEN}您的密钥文件: ${KEY_FILE}${NC}\n"
    else
        printf "${RED}证书未能下发，请检查 Cloudflare API 密钥和域名配置。${NC}\n"
    fi
}
